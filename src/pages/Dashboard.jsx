...
export default function Component() {
  ...
  const categories = ["All", ...new Set(Object.values(expenseData[selectedYear]).flatMap(month => Object.keys(month)))]
  ...
  const handlePieClick = (data) => {
    ...
    const subCategories = Object.keys(expenseData[selectedYear]["January"]).filter(cat => cat.startsWith(`${data.name}.`))
  ...
  return (
    <Card className="w-full max-w-4xl">
      <CardHeader>
        <CardTitle>Dynamic Expense Tracker</CardTitle>
        <CardDescription>Analyze your spending patterns with interactive charts</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <ChartContainer config={{}} className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={getFilteredData()}>
                ...
                <ChartTooltip content={<ChartTooltipContent />} />
                <Legend />
                {categories.filter(cat => cat !== "All" && (selectedCategory === "All" || cat === selectedCategory)).map((category, index) => (
